/* Lubayd SA V22.4.0 - capa de datos y sincronización robusta */
(function () {
  'use strict';
  ...
  async function save(type, payload, options = {}) {
    ...
    const verification = await window.LubaydOffline.get('records', id).catch(() => null);
    if (!verification?.id) throw new Error('El registro no pudo verificarse en el almacenamiento del teléfono.');
  ...
  async function syncOne(id) {
    ...
    const cloudReady = await window.LubaydCloud?.ensureReady?.(18000);
    if (!cloudReady) throw new Error('No se pudo activar la conexión para sincronizar.');
  ...
  async function saveFuel(movement) {
    ...
    try {
      ...
      await window.LubaydOffline.setCache('fuel_state', current).catch(() => {});
      emit('lubayd-fuel-state-changed', { state: current });
      throw error;
    }
  }
  ...
  window.addEventListener('online', async () => {
    ...
    await window.LubaydCloud?.ensureReady?.(18000).catch(() => false);
    syncAll().catch(error => console.warn('[Lubayd] Reintento online:', error));
  });

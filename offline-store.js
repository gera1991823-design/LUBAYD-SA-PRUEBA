...
(function () {
  ...
  async function mergeFuelCloudData(loads = [], movements = []) {
    for (const load of loads) await put(STORES.fuelLoads, { ...clone(load), syncStatus: 'synced', syncError: '', updatedAtLocal: nowIso() });
    for (const movement of movements) await put(STORES.fuelMovements, { ...clone(movement), syncStatus: 'synced', syncError: '', updatedAtLocal: nowIso() });
    emitChange();

/* Lubayd SA V21.0 - acceso local, asistencia, partes y combustible sin conexion */
(function () {
  'use strict';
  ...
  const STORES = {
    ...
    fuelLoads: 'fuel_loads',
    fuelMovements: 'fuel_movements',
    fuelQueue: 'fuel_queue',
    fuelFlowState: 'fuel_flow_state',
    fuelFlowMovements: 'fuel_flow_movements',
    fuelFlowQueue: 'fuel_flow_queue'
  };
  ...
  function openDatabase() {
    ...
    databasePromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = event => {
        ...
        if (!db.objectStoreNames.contains(STORES.fuelLoads)) {
          const store = db.createObjectStore(STORES.fuelLoads, { keyPath: 'id' });
          store.createIndex('status', 'status', { unique: false });
          store.createIndex('createdAt', 'createdAt', { unique: false });
        }
        if (!db.objectStoreNames.contains(STORES.fuelMovements)) {
          const store = db.createObjectStore(STORES.fuelMovements, { keyPath: 'id' });
          store.createIndex('loadId', 'loadId', { unique: false });
          store.createIndex('userId', 'createdByUid', { unique: false });
          store.createIndex('dateKey', 'dateKey', { unique: false });
        }
        if (!db.objectStoreNames.contains(STORES.fuelQueue)) {
          const store = db.createObjectStore(STORES.fuelQueue, { keyPath: 'recordId' });
          store.createIndex('userId', 'userId', { unique: false });
          store.createIndex('status', 'status', { unique: false });
          store.createIndex('createdAt', 'createdAt', { unique: false });
        }
        if (!db.objectStoreNames.contains(STORES.fuelFlowState)) {
          db.createObjectStore(STORES.fuelFlowState, { keyPath: 'id' });
        }
        if (!db.objectStoreNames.contains(STORES.fuelFlowMovements)) {
          const store = db.createObjectStore(STORES.fuelFlowMovements, { keyPath: 'id' });
          store.createIndex('dateKey', 'dateKey', { unique: false });
          store.createIndex('kind', 'kind', { unique: false });
          store.createIndex('userId', 'createdByUid', { unique: false });
        }
        if (!db.objectStoreNames.contains(STORES.fuelFlowQueue)) {
          const store = db.createObjectStore(STORES.fuelFlowQueue, { keyPath: 'recordId' });
          store.createIndex('userId', 'userId', { unique: false });
          store.createIndex('status', 'status', { unique: false });
          store.createIndex('createdAt', 'createdAt', { unique: false });
        }
  ...
  const OFFLINE_FUEL_SYNC_URL = 'https://southamerica-east1-lubayd-sa.cloudfunctions.net/syncFuelRecord';
  const FUEL_STATE_URL = 'https://southamerica-east1-lubayd-sa.cloudfunctions.net/getFuelState';

  async function saveFuelLoad(load) {
    if (!load?.id) throw new Error('La carga de combustible no tiene identificador.');
    await put(STORES.fuelLoads, { ...clone(load), updatedAtLocal: nowIso() });
    emitChange();
    return get(STORES.fuelLoads, load.id);
  }

  function getFuelLoad(id) {
    return get(STORES.fuelLoads, id);
  }

  async function listFuelLoads() {
    const items = await getAll(STORES.fuelLoads);
    return items.sort((a, b) => String(b.createdAt || b.dateTime || '').localeCompare(String(a.createdAt || a.dateTime || '')));
  }

  async function saveFuelMovement(movement) {
    if (!movement?.id || !movement?.loadId) throw new Error('El movimiento de combustible no tiene identificadores válidos.');
    await put(STORES.fuelMovements, { ...clone(movement), updatedAtLocal: nowIso() });
    emitChange();
    return get(STORES.fuelMovements, movement.id);
  }

  function getFuelMovement(id) {
    return get(STORES.fuelMovements, id);
  }

  async function listFuelMovements(options = {}) {
    const items = await getAll(STORES.fuelMovements);
    return items
      .filter(item => !options.loadId || item.loadId === options.loadId)
      .sort((a, b) => String(b.createdAt || b.dateTime || '').localeCompare(String(a.createdAt || a.dateTime || '')));
  }

  async function mergeFuelCloudData(loads = [], movements = []) {
    for (const load of loads) await put(STORES.fuelLoads, { ...clone(load), syncStatus: 'synced', syncError: '', serverConfirmed: true, updatedAtLocal: nowIso() });
    for (const movement of movements) await put(STORES.fuelMovements, { ...clone(movement), syncStatus: 'synced', syncError: '', serverConfirmed: true, updatedAtLocal: nowIso() });
    emitChange();
    return { loads: await listFuelLoads(), movements: await listFuelMovements() };
  }

  async function enqueueFuelOperation(payload) {
    const record = clone(payload?.record || {});
    const recordId = String(payload?.recordId || record.id || '').trim();
    const userId = String(payload?.userId || record.createdByUid || '').trim();
    if (!recordId || !userId || !record.kind) throw new Error('El registro de combustible no tiene datos válidos.');
    const current = await get(STORES.fuelQueue, recordId);
    const item = {
      recordId,
      userId,
      operation: 'upsert',
    ...
    await put(STORES.fuelQueue, item);
    emitChange();
    return item;
  }

  function getFuelQueueItem(recordId) {
    return get(STORES.fuelQueue, recordId);
  }

  async function listFuelQueue(options = {}) {
    const items = await getAll(STORES.fuelQueue);
    return items
      .filter(item => !options.userId || item.userId === options.userId)
      .filter(item => !options.statuses || options.statuses.includes(item.status))
      .sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
  ...
  async function updateFuelQueue(recordId, changes) {
    const item = await getFuelQueueItem(recordId);
    if (!item) return null;
    Object.assign(item, clone(changes || {}), { updatedAt: nowIso() });
    await put(STORES.fuelQueue, item);
    emitChange();
  ...
  async function markFuelSynced(recordId) {
    await remove(STORES.fuelQueue, recordId).catch(() => {});
    emitChange();
  }

  async function markFuelError(recordId, error) {
    const item = await getFuelQueueItem(recordId);
    if (!item) return;
    item.status = 'error';
    item.attempts = Number(item.attempts || 0) + 1;
  ...
  async function syncFuelQueueItem(item, identity, options = {}) {
    const idToken = String(options?.idToken || '').trim();
    const hasDeviceCredential = Boolean(identity?.deviceId && identity?.deviceToken);
    if (!idToken && !hasDeviceCredential) throw new Error('Inicia sesión con el mismo operador o prepara este dispositivo para sincronizar combustible.');
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 60000);
    try {
      const headers = { 'Content-Type': 'application/json' };
      if (idToken) headers.Authorization = `Bearer ${idToken}`;
      const response = await fetch(OFFLINE_FUEL_SYNC_URL, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          deviceId: identity?.deviceId || '',
          deviceToken: identity?.deviceToken || '',
          item,
          parentLoad: options?.parentLoad || null
        }),
        signal: controller.signal
      });
      let result = {};
      try { result = await response.json(); } catch (_) { result = {}; }
      if (!response.ok || result.ok !== true) throw new Error(result.error || `No se pudo sincronizar combustible (${response.status}).`);
      return result;
    } catch (error) {
      if (error?.name === 'AbortError') throw new Error('La sincronización de combustible demoró demasiado.');
      throw error;
    } finally {
      clearTimeout(timer);
    }
  ...
  async function fetchFuelCloudState(idToken) {
    const token = String(idToken || '').trim();
    if (!token) throw new Error('Debes iniciar sesión online para consultar el combustible del servidor.');
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 60000);
    try {
      const response = await fetch(FUEL_STATE_URL, {
        method: 'POST',
        headers: {
      ...
      let result = {};
      try { result = await response.json(); } catch (_) { result = {}; }
      if (!response.ok || result.ok !== true) throw new Error(result.error || `No se pudo consultar combustible (${response.status}).`);
      return {
        loads: Array.isArray(result.loads) ? result.loads : [],
        movements: Array.isArray(result.movements) ? result.movements : [],
        state: result.state || null
      };
    } catch (error) {
      if (error?.name === 'AbortError') throw new Error('La consulta de combustible demoró demasiado.');
      throw error;
    } finally {
      clearTimeout(timer);
    }
  ...
  const OFFLINE_FUEL_FLOW_SYNC_URL = 'https://southamerica-east1-lubayd-sa.cloudfunctions.net/syncFuelFlowRecord';
  const FUEL_FLOW_STATE_URL = 'https://southamerica-east1-lubayd-sa.cloudfunctions.net/getFuelFlowState';

  async function saveFuelFlowState(state, key = 'local') {
    const value = { ...clone(state || {}), id: String(key || state?.id || 'local'), updatedAtLocal: nowIso() };
    await put(STORES.fuelFlowState, value);
    emitChange();
    return get(STORES.fuelFlowState, value.id);
  }

  function getFuelFlowState(key = 'local') {
    return get(STORES.fuelFlowState, String(key || 'local'));
  }

  async function saveFuelFlowMovement(movement) {
    if (!movement?.id || !movement?.kind) throw new Error('El movimiento de combustible no tiene identificadores válidos.');
    const value = { ...clone(movement), updatedAtLocal: nowIso() };
    await put(STORES.fuelFlowMovements, value);
    emitChange();
    return get(STORES.fuelFlowMovements, value.id);
  }

  function getFuelFlowMovement(id) {
    return get(STORES.fuelFlowMovements, id);
  }

  async function listFuelFlowMovements(options = {}) {
    const items = await getAll(STORES.fuelFlowMovements);
    return items
      .filter(item => !options.dateKey || item.dateKey === options.dateKey)
  ...
  async function mergeFuelFlowCloudData(serverState = {}, movements = []) {
    await saveFuelFlowState({ ...clone(serverState), serverConfirmed: true }, 'server');
    for (const movement of movements || []) {
      if (!movement?.id) continue;
      const local = await getFuelFlowMovement(movement.id).catch(() => null);
      await put(STORES.fuelFlowMovements, {
        ...(local || {}),
        ...clone(movement),
        proofDataUrl: local?.proofDataUrl || movement.proofDataUrl || '',
        syncStatus: 'synced',
        syncError: '',
    ...
    emitChange();
    return {
      state: await getFuelFlowState('server'),
      movements: await listFuelFlowMovements()
    };
  }

  async function enqueueFuelFlowOperation(payload) {
    const record = clone(payload?.record || {});
    const recordId = String(payload?.recordId || record.id || '').trim();
    const userId = String(payload?.userId || record.createdByUid || '').trim();
    if (!recordId || !userId || !record.kind) throw new Error('El movimiento de combustible no tiene datos válidos.');
    const current = await get(STORES.fuelFlowQueue, recordId);
    const item = {
      recordId,
      userId,
      operation: 'upsert',
    ...
    await put(STORES.fuelFlowQueue, item);
    emitChange();
    return item;
  }

  function getFuelFlowQueueItem(recordId) {
    return get(STORES.fuelFlowQueue, recordId);
  }

  async function listFuelFlowQueue(options = {}) {
    const items = await getAll(STORES.fuelFlowQueue);
    return items
      .filter(item => !options.userId || item.userId === options.userId)
      .filter(item => !options.statuses || options.statuses.includes(item.status))
      .sort((a, b) => String(a.createdAt || '').localeCompare(String(b.createdAt || '')));
  ...
  async function updateFuelFlowQueue(recordId, changes) {
    const item = await getFuelFlowQueueItem(recordId);
    if (!item) return null;
    Object.assign(item, clone(changes || {}), { updatedAt: nowIso() });
    await put(STORES.fuelFlowQueue, item);
    emitChange();
  ...
  async function markFuelFlowSynced(recordId) {
    const movement = await getFuelFlowMovement(recordId).catch(() => null);
    if (movement) await put(STORES.fuelFlowMovements, { ...movement, syncStatus: 'synced', syncError: '', serverConfirmed: true, updatedAtLocal: nowIso() });
    await remove(STORES.fuelFlowQueue, recordId).catch(() => {});
    emitChange();
  }

  async function markFuelFlowError(recordId, error) {
    const item = await getFuelFlowQueueItem(recordId);
    if (item) {
      item.status = 'error';
      item.attempts = Number(item.attempts || 0) + 1;
    ...
    const movement = await getFuelFlowMovement(recordId).catch(() => null);
    if (movement) await put(STORES.fuelFlowMovements, { ...movement, syncStatus: 'error', syncError: error?.message || String(error || 'Error desconocido'), serverConfirmed: false, updatedAtLocal: nowIso() });
    emitChange();
  }

  async function syncFuelFlowQueueItem(item, identity, options = {}) {
    const idToken = String(options?.idToken || '').trim();
    const hasDeviceCredential = Boolean(identity?.deviceId && identity?.deviceToken);
    if (!idToken && !hasDeviceCredential) throw new Error('Inicia sesión online o prepara este dispositivo para sincronizar combustible.');
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 60000);
    try {
      const headers = { 'Content-Type': 'application/json' };
      if (idToken) headers.Authorization = `Bearer ${idToken}`;
      const response = await fetch(OFFLINE_FUEL_FLOW_SYNC_URL, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          deviceId: identity?.deviceId || '',
      ...
      let result = {};
      try { result = await response.json(); } catch (_) { result = {}; }
      if (!response.ok || result.ok !== true) throw new Error(result.error || `No se pudo sincronizar el movimiento (${response.status}).`);
      return result;
    } catch (error) {
      if (error?.name === 'AbortError') throw new Error('La sincronización de combustible demoró demasiado.');
      throw error;
  ...
  async function fetchFuelFlowCloudState(idToken) {
    const token = String(idToken || '').trim();
    if (!token) throw new Error('Debes iniciar sesión online para consultar el combustible del servidor.');
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 60000);
    try {
      const response = await fetch(FUEL_FLOW_STATE_URL, {
        method: 'POST',
      ...
      let result = {};
      try { result = await response.json(); } catch (_) { result = {}; }
      if (!response.ok || result.ok !== true) throw new Error(result.error || `No se pudo consultar el flujo de combustible (${response.status}).`);
      return {
        state: result.state || null,
        movements: Array.isArray(result.movements) ? result.movements : []
      };
  ...
  window.LubaydOffline = {
    ...
    saveFuelFlowState,
    getFuelFlowState,
    saveFuelFlowMovement,
    getFuelFlowMovement,
    listFuelFlowMovements,
    mergeFuelFlowCloudData,
    enqueueFuelFlowOperation,
    getFuelFlowQueueItem,
    listFuelFlowQueue,
    updateFuelFlowQueue,
    markFuelFlowSynced,
    markFuelFlowError,
    syncFuelFlowQueueItem,
    fetchFuelFlowCloudState
  };
})();

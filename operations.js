...
(function () {
  ...
  async function provisionOfflineUser(uid, pinInput) {
    ...
    try {
      ...
      const identity = await window.LubaydOffline.getDeviceIdentity().catch(() => null);
      if (identity?.enrolled && navigator.onLine) await enrollOfflineDevice({ silent: true });
      await renderUsers();
  ...
  function bindEvents() {
    ...
    $('#forceSyncBtn')?.addEventListener('click', async () => { if (navigator.onLine) await Promise.all([window.LubaydSyncOfflineAttendance?.({ silent: true }), window.LubaydSyncOfflineParts?.({ silent: true })]); renderSync(); notify('Sincronización solicitada', navigator.onLine ? 'Se procesaron las marcas y partes pendientes.' : 'El dispositivo continúa sin conexión.', navigator.onLine ? 'success' : 'warning'); });

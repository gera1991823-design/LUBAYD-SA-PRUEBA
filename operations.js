...
(function () {
  ...
  async function renderOfflineDeviceStatus() {
    const status = $('#offlineDeviceStatus');
    const nameInput = $('#offlineDeviceName');
    if (!window.LubaydOffline?.available) {
      if (status) status.textContent = 'No compatible';
      return;
    }
    const identity = await window.LubaydOffline.getDeviceIdentity().catch(() => null);
    const profiles = await window.LubaydOffline.listProfiles({ onlyWithPin: true, onlyOperators: true }).catch(() => []);
    if (nameInput && !nameInput.value) nameInput.value = identity?.deviceName || `Dispositivo ${new Date().toLocaleDateString('es-UY')}`;
    if (status) {
      status.textContent = identity?.enrolled ? `Habilitado · ${profiles.length} operador(es)` : `Pendiente · ${profiles.length} operador(es)`;
      status.classList.toggle('ready', Boolean(identity?.enrolled && profiles.length));
    }
  }

  async function enrollOfflineDevice(options = {}) {
    if (!isAdmin()) return;
    const button = $('#enrollOfflineDeviceBtn');
    const message = $('#offlineDeviceMessage');
    if (button) button.disabled = true;
    if (message) message.textContent = '';
    try {
      if (!navigator.onLine) throw new Error('Necesitas internet solamente para preparar o actualizar el dispositivo.');
      if (!window.LubaydOfflineDeviceCloud?.available) throw new Error('Firebase no está disponible para habilitar el dispositivo.');
      const payload = await window.LubaydOffline.getDeviceEnrollmentPayload($('#offlineDeviceName')?.value || 'Dispositivo Lubayd');
      await window.LubaydOfflineDeviceCloud.enrollDevice(payload);
      await window.LubaydOffline.markDeviceEnrolled({
        deviceName: payload.deviceName,
        allowedUserIds: payload.allowedUserIds,
        preparedByUid: state.user?.uid || ''
      });
      await renderOfflineDeviceStatus();
      if (!options.silent) notify('Dispositivo habilitado', `${payload.allowedUserIds.length} operador(es) pueden ingresar y sincronizar sin una sesión de Firebase.`);
      if (message) {
        message.textContent = 'Preparación completa. Puedes llevar este dispositivo a una zona sin señal.';
      ...
      if (!options.silent) notify('No se pudo habilitar', error.message || String(error), 'error');
      throw error;
    } finally {
  ...
  async function provisionOfflineUser(uid, pinInput) {
    const user = state.users.find(item => item.id === uid);
    if (!user) return notify('Usuario no encontrado', 'Actualiza la lista e intenta nuevamente.', 'error');
    const pin = String(pinInput?.value || '').trim();
    try {
      await window.LubaydOffline.provisionProfile({
        uid: user.id,
        nombre: user.nombre || user.email || 'Operador',
        email: user.email || '',
        role: user.role || 'operador',
        active: user.active === true
      }, pin, {
        preparedByUid: state.user?.uid || '',
        preparedByName: state.profile?.nombre || state.user?.email || 'Administrador'
      });
      if (pinInput) pinInput.value = '';
      if (navigator.onLine) await enrollOfflineDevice({ silent: true });
      await renderUsers();
      notify('Operador preparado', `${user.nombre || user.email} ya puede ingresar sin internet en este dispositivo.`);
    } catch (error) {
      notify('No se pudo preparar', error.message || String(error), 'error');
    }
  }

  async function removeOfflineUser(uid) {
    const user = state.users.find(item => item.id === uid);
    if (!confirm(`¿Quitar el acceso offline de ${user?.nombre || user?.email || 'este usuario'} en este dispositivo?`)) return;
    try {
      await window.LubaydOffline.removeProvisionedProfile(uid);
      const identity = await window.LubaydOffline.getDeviceIdentity().catch(() => null);
      if (identity?.enrolled && navigator.onLine) {
        const profiles = await window.LubaydOffline.listProfiles({ onlyWithPin: true, onlyOperators: true });
        if (profiles.length) await enrollOfflineDevice({ silent: true });
        else await window.LubaydOfflineDeviceCloud?.revokeDevice?.(identity.deviceId);
      }
      await renderUsers();
      notify('Acceso eliminado', 'El operador ya no aparece en el inicio de sesión local.');
    } catch (error) {
      notify('No se pudo quitar', error.message || String(error), 'error');
    }
  }

  async function revokeOfflineDevice() {
    if (!isAdmin()) return;
    const identity = await window.LubaydOffline?.getDeviceIdentity?.().catch(() => null);
    if (!identity?.deviceId || !identity.enrolled) return notify('Dispositivo no habilitado', 'No hay una autorización activa para revocar.', 'warning');
    if (!confirm('¿Revocar este dispositivo? Las marcas pendientes seguirán guardadas, pero no podrán sincronizarse hasta habilitarlo nuevamente.')) return;
    try {
      await window.LubaydOfflineDeviceCloud.revokeDevice(identity.deviceId);
      await window.LubaydOffline.markDeviceRevoked();
      await renderOfflineDeviceStatus();
      notify('Dispositivo revocado', 'El acceso local permanece, pero la sincronización quedó bloqueada.');
    } catch (error) {
      notify('No se pudo revocar', error.message || String(error), 'error');
    }
  }
  ...
  async function renderUsers() {
    ...
    const localProfiles = await window.LubaydOffline?.listProfiles?.({ onlyWithPin: true, onlyOperators: false }).catch(() => []) || [];
    const localByUid = new Map(localProfiles.map(profile => [profile.uid, profile]));
    const list = state.users.filter(user => {
      const matchesSearch = [user.nombre, user.email].join(' ').toLowerCase().includes(search);
      const matchesRole = !role || user.role === role;
      const matchesStatus = !statusFilter || (statusFilter === 'active' ? user.active === true : user.active !== true);
    ...
    root.innerHTML = list.length ? list.map(user => {
      const localReady = localByUid.has(user.id);
      const offlineControls = user.role === 'operador' && user.active === true
        ? `<div class="offline-user-controls"><label><span>PIN en este dispositivo</span><input type="password" inputmode="numeric" pattern="[0-9]*" maxlength="6" data-offline-pin="${escapeHtml(user.id)}" placeholder="4 a 6 números"></label><button type="button" class="btn btn-soft" data-offline-provision="${escapeHtml(user.id)}">${localReady ? 'Cambiar PIN' : 'Preparar'}</button>${localReady ? `<button type="button" class="btn btn-ghost offline-remove-user" data-offline-remove="${escapeHtml(user.id)}">Quitar</button>` : ''}<span class="offline-user-state ${localReady ? 'ready' : ''}"><i></i>${localReady ? 'Acceso local listo' : 'Sin acceso local'}</span></div>`
        : '<span class="offline-user-state"><i></i>Solo operadores activos pueden marcar offline</span>';
      return `<article class="user-admin-row"><div class="user-admin-avatar">${initials(user.nombre || user.email)}</div><div class="user-admin-identity"><strong>${escapeHtml(user.nombre || 'Sin nombre')}</strong><span>${escapeHtml(user.email || 'Sin correo')}</span>${offlineControls}</div><label><span>Rol</span><select data-user-role="${escapeHtml(user.id)}" ${user.id === state.user?.uid ? 'disabled' : ''}><option value="operador" ${user.role === 'operador' ? 'selected' : ''}>Operador</option><option value="supervisor" ${user.role === 'supervisor' ? 'selected' : ''}>Supervisor</option><option value="admin" ${user.role === 'admin' ? 'selected' : ''}>Administrador</option></select></label><label class="user-active-toggle"><span>${user.active === true ? 'Activo' : 'Desactivado'}</span><input type="checkbox" data-user-active="${escapeHtml(user.id)}" ${user.active === true ? 'checked' : ''} ${user.id === state.user?.uid ? 'disabled' : ''}></label><div class="user-admin-state ${user.active === true ? 'active' : 'inactive'}"><i></i>${user.active === true ? 'Habilitado' : 'Bloqueado'}</div></article>`;
    }).join('') : '<div class="empty-state">No hay usuarios que coincidan con los filtros.</div>';
    ...
    root.querySelectorAll('[data-offline-provision]').forEach(button => button.addEventListener('click', () => { const input = Array.from(root.querySelectorAll('[data-offline-pin]')).find(node => node.dataset.offlinePin === button.dataset.offlineProvision); provisionOfflineUser(button.dataset.offlineProvision, input); }));
    root.querySelectorAll('[data-offline-remove]').forEach(button => button.addEventListener('click', () => removeOfflineUser(button.dataset.offlineRemove)));
    await renderOfflineDeviceStatus();
  }
  ...
  async function updateUser(uid, patch) {
    try {
      ...
      await window.LubaydOffline?.updateProfileState?.(uid, patch).catch(() => {});
      const identity = await window.LubaydOffline?.getDeviceIdentity?.().catch(() => null);
      const profiles = await window.LubaydOffline?.listProfiles?.({ onlyWithPin: true, onlyOperators: true }).catch(() => []) || [];
      if (identity?.enrolled && navigator.onLine) {
        if (profiles.length) await enrollOfflineDevice({ silent: true });
  ...
  function bindEvents() {
    ...
    $('#enrollOfflineDeviceBtn')?.addEventListener('click', () => enrollOfflineDevice());
    $('#syncOfflineDeviceBtn')?.addEventListener('click', async () => { try { await window.LubaydSyncOfflineAttendance?.({ silent: false }); await renderOfflineDeviceStatus(); } catch (error) { notify('No se pudo sincronizar', error.message || String(error), 'error'); } });
    $('#revokeOfflineDeviceBtn')?.addEventListener('click', revokeOfflineDevice);
    ...
    $('#forceSyncBtn')?.addEventListener('click', async () => {
      const results = await Promise.allSettled([
        ...
        window.LubaydSyncOfflineParts?.({ silent: true }),
        window.LubaydSyncFuel?.({ silent: true })
      ]);
      renderSync();
      const failed = results.some(result => result.status === 'rejected');
      notify('Sincronización solicitada', failed ? 'No hubo respuesta de internet; los datos continúan guardados y se reintentarán.' : 'Se procesaron las marcas, partes y registros de combustible pendientes.', failed ? 'warning' : 'success');
    });
    ...
    $('#resetAppCacheBtn')?.addEventListener('click', async () => {
      notify('Buscando actualización', 'La aplicación comprobará y aplicará la versión más reciente.');

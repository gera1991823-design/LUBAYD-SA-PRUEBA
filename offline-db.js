/* Lubayd SA V21.3.0 - autenticacion y datos offline */
(function () {
  'use strict';
  const { config, normalizeEmail, clone, uid } = window.Lubayd;
  const DB_NAME = 'lubayd-sa-v21-3';
  const DB_VERSION = 1;
  const SESSION_KEY = 'lubayd_offline_session_v21_3';
  ...
  open().then(resetInterrupted).catch(error => console.error('[Lubayd Offline]', error));
  window.LubaydOffline = {
    available: Boolean(window.indexedDB && window.crypto?.subtle),
    open, put, get, remove, getAll, getByIndex,
    ...
    setCache, getCache, setSetting, getSetting, status
  };
})();

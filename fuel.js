/* Lubayd SA V20.9.2 - combustible cloud verificado y conciliacion multidispositivo */
(function () {
  'use strict';

  const VERSION = '20.9.2';
  ...
  function activeLoad() {
    // V20.9.2 mantiene un único control continuo. Si una versión anterior lo cerró al llegar a cero,
  ...
  async function refreshLocal() {
    ...
    const [allLoads, allMovements] = await Promise.all([
      window.LubaydOffline.listFuelLoads(),
      window.LubaydOffline.listFuelMovements()
    ]);
    const cloudOnly = !isOffline() && ['admin', 'supervisor'].includes(role());
    loads = cloudOnly ? allLoads.filter(item => item.serverConfirmed === true) : allLoads;
    movements = cloudOnly ? allMovements.filter(item => item.serverConfirmed === true) : allMovements;
    render();
  }
  ...
  async function fetchAndMergeCloudState() {
    ...
    if (isOffline() || !user?.uid || !window.LubaydOffline?.fetchFuelCloudState) return null;
    const idToken = await user.getIdToken(false);
    const data = await window.LubaydOffline.fetchFuelCloudState(idToken);
    await window.LubaydOffline.mergeFuelCloudData(data.loads || [], data.movements || []);
    await refreshLocal();
    return data;
  }

  async function rebuildMissingFuelQueueFromLocal(cloudData) {
    if (!isOperator() || isOffline() || !onlineUser()?.uid || !cloudData) return 0;
    const currentUid = onlineUser().uid;
    const [allLoads, allMovements, queue] = await Promise.all([
      window.LubaydOffline.listFuelLoads(),
      window.LubaydOffline.listFuelMovements(),
      window.LubaydOffline.listFuelQueue({ statuses: ['pending', 'error', 'syncing'] })
    ]);
    const queued = new Set(queue.map(item => item.recordId));
    const cloudLoadIds = new Set((cloudData.loads || []).map(item => item.id));
    const cloudMovementIds = new Set((cloudData.movements || []).map(item => item.id));
    let rebuilt = 0;

    for (const load of allLoads) {
      if (cloudLoadIds.has(load.id) || queued.has(load.id)) continue;
      if (load.createdByUid !== currentUid || load.kind !== 'fuel_load' || !load.proofDataUrl) continue;
      const record = { ...load, syncStatus: 'pending', syncError: '', serverConfirmed: false };
      await window.LubaydOffline.saveFuelLoad(record);
      await window.LubaydOffline.enqueueFuelOperation({ recordId: record.id, userId: record.createdByUid, record });
      queued.add(record.id);
      rebuilt += 1;
    }

    for (const movement of allMovements) {
      if (cloudMovementIds.has(movement.id) || queued.has(movement.id)) continue;
      if (movement.createdByUid !== currentUid || !['fuel_refill', 'fuel_check', 'fuel_usage'].includes(movement.kind) || !movement.proofDataUrl) continue;
      const record = { ...movement, syncStatus: 'pending', syncError: '', serverConfirmed: false };
      await window.LubaydOffline.saveFuelMovement(record);
      await window.LubaydOffline.enqueueFuelOperation({ recordId: record.id, userId: record.createdByUid, record });
      queued.add(record.id);
      rebuilt += 1;
    }
    if (rebuilt) await refreshLocal();
    return rebuilt;
  }
  ...
  async function syncFuelQueue(options = {}) {
    ...
    syncPromise = (async () => {
      ...
      let initialCloud = null;
      if (hasOnlineOperator) {
        try {
          initialCloud = await fetchAndMergeCloudState();
          await rebuildMissingFuelQueueFromLocal(initialCloud);
        } catch (error) {
          console.warn('No se pudo verificar combustible antes de sincronizar:', error);
        }
      }
      ...
      for (const item of items) {
        try {
          ...
          if (result.load) await window.LubaydOffline.saveFuelLoad({ ...result.load, syncStatus: 'syncing', syncError: '', serverConfirmed: false });
          if (result.movement) await window.LubaydOffline.saveFuelMovement({ ...result.movement, syncStatus: 'syncing', syncError: '', serverConfirmed: false });
          submitted.push(item);
        } catch (error) {
          errors += 1;
          await window.LubaydOffline.markFuelError(item.recordId, error);
          if (item.record?.kind === 'fuel_load') await window.LubaydOffline.saveFuelLoad({ ...item.record, syncStatus: 'error', syncError: error.message || String(error), serverConfirmed: false });
          if (['fuel_usage', 'fuel_check', 'fuel_refill'].includes(item.record?.kind)) await window.LubaydOffline.saveFuelMovement({ ...item.record, syncStatus: 'error', syncError: error.message || String(error), serverConfirmed: false });
        }
      }

      let cloudData = null;
      try {
        cloudData = hasOnlineOperator ? await fetchAndMergeCloudState() : null;
      } catch (error) {
        console.warn('No se pudo confirmar combustible en el servidor:', error);
      }
      ...
      for (const item of submitted) {
        const confirmed = item.record?.kind === 'fuel_load'
          ? cloudLoadIds.has(item.recordId)
          : cloudMovementIds.has(item.recordId);
        if (confirmed) {
          await window.LubaydOffline.markFuelSynced(item.recordId);
          synced += 1;
        } else {
          errors += 1;
          const error = new Error('El servidor no confirmó este registro. Se mantendrá pendiente para volver a intentarlo.');
          await window.LubaydOffline.markFuelError(item.recordId, error);
          if (item.record?.kind === 'fuel_load') await window.LubaydOffline.saveFuelLoad({ ...item.record, syncStatus: 'error', syncError: error.message, serverConfirmed: false });
          if (['fuel_usage', 'fuel_check', 'fuel_refill'].includes(item.record?.kind)) await window.LubaydOffline.saveFuelMovement({ ...item.record, syncStatus: 'error', syncError: error.message, serverConfirmed: false });
        }
      }

      await refreshLocal();
      if (!options.silent && (synced || errors)) notify(errors ? 'Sincronización incompleta' : 'Combustible sincronizado', errors ? `${synced} confirmados por el servidor y ${errors} pendientes.` : `${synced} registro(s) confirmados y visibles para administración.`, errors ? 'error' : 'success');

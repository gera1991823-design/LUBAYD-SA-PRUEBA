...
(function () {
  ...
  async function getRegistration() {
    ...
    if (!registration) {
      registration = await navigator.serviceWorker.register('./service-worker.js?v=20.7.0', {
        scope: './',
        updateViaCache: 'none'
      });
    }
  ...
  async function registerFcmToken() {
    ...
    const saved = await withTimeout(
      saveToken(token),
      20000,
      'El token se generó, pero no pudo guardarse en Firestore. Publica las reglas V20.7.'
    );

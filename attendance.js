...
(function () {
  ...
  async function updateOfflineState() {
    ...
    const canSync = navigator.onLine && (hasOnlineFirebaseSession() || Boolean(identity));
    const box = $('#attendanceQueueState');
    if (box) {
      box.className = `attendance-queue-state ${errors.length ? 'error' : items.length ? 'pending' : ''}`;
      box.innerHTML = errors.length
        ? `<i></i><span><strong>${errors.length} marca(s) con error</strong><small>Conéctate e intenta sincronizar nuevamente.</small></span>`
        : items.length
          ? `<i></i><span><strong>${items.length} marca(s) pendiente(s)</strong><small>${identity ? 'El dispositivo las enviará automáticamente al recuperar internet.' : 'El administrador debe preparar este dispositivo para sincronización automática.'}</small></span>`
          : '<i></i><span><strong>Todo sincronizado</strong><small>Las marcas ya están disponibles para administración.</small></span>';
    }
    const button = $('#attendanceSyncBtn');
    if (button) {
      button.disabled = state.syncing || !items.length || !canSync;
      button.innerHTML = state.syncing
        ? '<span class="button-spinner"></span> Sincronizando...'
        : `<svg><use href="#i-refresh"></use></svg> Sincronizar ahora${items.length ? ` (${items.length})` : ''}`;
    }
  }

  async function syncPending(options = {}) {
    if (state.syncing || !navigator.onLine || !window.LubaydOffline?.available) {
    ...
    let synced = 0;
    try {
      const items = await window.LubaydOffline.listQueue({ userId, statuses: ['pending', 'error'] });
      for (const item of items) {
        try {
          await window.LubaydOffline.updateQueue(item.id, { status: 'syncing', lastError: '' });
          let remote;
          const canUseFirebaseUser = onlineUid === item.userId && window.LubaydCurrentProfile?.role === 'operador' && window.LubaydAttendanceData?.available;
          ...
          if (!navigator.onLine || /network|conexi|offline|fetch/i.test(String(error?.message || error))) break;
        }
      }
...
La acción quedará registrada y no se puede deshacer.`);
  ...
  async function openCamera(mode) {
    ...
    const record = currentTodayRecord();
    if (mode === 'entry' && record?.entryAt) return notify('Llegada ya registrada', 'Solo se permite una llegada por jornada.', 'error');
  ...
  async function confirmMark() {
    if (state.busy || !state.photoBlob || !state.gps || !canRegister()) return;
    ...
    try {
      ...
      await loadLocalRecords();
      notify(type === 'entry' ? 'Llegada guardada' : 'Salida guardada', hasOnlineFirebaseSession()
  ...
  function bindEvents() {
    ...
    $('#attendanceSyncBtn')?.addEventListener('click', () => syncPending());
    ...
    window.addEventListener('online', () => { renderPersonal(); if (!state.offlineSession) subscribe(); syncPending({ silent: true, allUsers: true }); });
  window.addEventListener('load', () => { if (navigator.onLine) window.setTimeout(() => syncPending({ silent: true, allUsers: true }), 1800); });

...
(function () {
  ...
  async function updateOfflineState() {
    ...
    if (box) {
      ...
      box.innerHTML = errors.length
        ...
        : items.length
          ? `<i></i><span><strong>${items.length} marca(s) pendiente(s)</strong><small>Guardadas en este teléfono y aún no visibles para administración.</small></span>`
    ...
    if (button) {
      button.disabled = state.syncing || !items.length || !hasOnlineFirebaseSession();
  ...
  async function syncPending(options = {}) {
    if (state.syncing || !state.user?.uid || !hasOnlineFirebaseSession() || !window.LubaydOffline?.available) {
      await updateOfflineState();
      return;
    }
    state.syncing = true;
    await window.LubaydOffline.retryErrors(state.user.uid).catch(() => {});
    await updateOfflineState();
    let synced = 0;
    try {
      const items = await window.LubaydOffline.listQueue({ userId: state.user.uid, statuses: ['pending', 'error'] });
      for (const item of items) {
        try {
          await window.LubaydOffline.updateQueue(item.id, { status: 'syncing', lastError: '' });
          const remote = await window.LubaydAttendanceData.registerQueued(item);
          await window.LubaydOffline.markSynced(item.id, remote);
          synced += 1;
        } catch (error) {
          await window.LubaydOffline.markError(item.id, error);
          if (!navigator.onLine || /network|conexi|offline/i.test(String(error?.message || error))) break;
        }
      }
...
La acción quedará registrada y no se puede deshacer.`);
  ...
  async function openPhoto(path) {
    ...
    try {
      const local = await window.LubaydOffline?.getPhoto?.(path).catch(() => null);
      const url = local?.imageData || await window.LubaydAttendanceData.getPhotoUrl(path);
      if (image) {
        image.src = url;
  ...
  function bindEvents() {
    ...
    window.addEventListener('online', () => { renderPersonal(); if (!state.offlineSession) { subscribe(); syncPending({ silent: true }); } });

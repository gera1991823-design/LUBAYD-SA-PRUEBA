...
async function authorizeCurrentDeviceForOperator(options = {}) {
  if (!navigator.onLine || offlineSession || !hasOnlineFirebaseSession()) return null;
...
window.addEventListener('online', () => {
  syncNetworkStatus();
  updateOfflinePreparationUi();
  authorizeCurrentDeviceForOperator({ silent: true })
    .catch(() => null)
    .finally(() => syncPendingParts({ silent: true }).catch(console.warn));
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=20.8.4', { scope: './', updateViaCache: 'none' });

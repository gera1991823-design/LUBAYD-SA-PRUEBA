...
async function activateOfflineSession(user, profile) {
  offlineSession = true;
  ...
  setCloudStatus('Modo offline', false, 'Las marcas quedan pendientes en este teléfono');
  renderAll();
...
async function offlineLogin() {
  ...
  try {
    ...
    const preparedAt = new Date(profile.preparedAt || 0).getTime();
    if (!preparedAt || Date.now() - preparedAt > 7 * 24 * 60 * 60 * 1000) {
      throw new Error('La autorización offline venció. Conecta el teléfono e inicia sesión nuevamente.');
    }
    ...
    if (button) { button.disabled = false; button.innerHTML = '<svg><use href="#i-shield"></use></svg> Abrir modo offline'; }
  }
}
...
async function logout() {
  if (!confirm(offlineSession ? '¿Cerrar el modo offline?' : '¿Cerrar la sesión actual?')) return;
  if (offlineSession) {
  ...
  const pending = authenticatedUser?.uid && window.LubaydOffline?.available
    ? await window.LubaydOffline.pendingCount(authenticatedUser.uid).catch(() => 0)
    : 0;
  if (pending > 0) {
    showView('asistencia');
    showToast('Sincronización pendiente', `Hay ${pending} marca(s) guardada(s) en este teléfono. Sincroniza antes de cerrar sesión.`);
    return;
  }
...
async function handleAuthChange(user) {
  ...
  try {
    ...
    authenticatedProfile = profile;
    window.LubaydCurrentProfile = profile;
    if (window.LubaydOffline?.saveProfile) {
      await window.LubaydOffline.saveProfile(user, profile).catch(error => console.warn('Preparar perfil offline:', error));
    }
    document.body.classList.remove('auth-pending');
...
function showView(id) {
  ...
  if (offlineSession && id !== 'asistencia') {
    showToast('Modo offline', 'En una sesión por PIN solo está disponible Asistencia.');
    id = 'asistencia';
  }
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=20.6.1', { scope: './', updateViaCache: 'none' });

...
const state = {
  ...
  async saveRecord(record) {
    ...
    if (hasOnlineFirebaseSession() && window.LubaydCloud?.available) {
      try {
        setCloudStatus('Sincronizando…', false, 'Guardando el parte en Firebase');
        await window.LubaydCloud.save(pendingRecord);
        this.updateLocal(pendingRecord.id, { syncStatus: 'synced', syncError: '', syncedAtClient: new Date().toISOString() });
        await window.LubaydOffline?.removePartQueue?.(pendingRecord.id).catch(() => {});
        setCloudStatus('Sincronizado', true, `Actualizado ${formatTime(new Date())}`);
        return { ...pendingRecord, syncStatus: 'synced' };
      } catch (error) {
        console.warn('El parte quedará pendiente:', error);
      }
    }
  ...
  async deleteRecord(id) {
    ...
    if (hasOnlineFirebaseSession() && window.LubaydCloud?.available) {
      try {
        setCloudStatus('Sincronizando…', false, 'Eliminando el parte');
        await window.LubaydCloud.remove(id);
        await window.LubaydOffline?.removePartQueue?.(id).catch(() => {});
        setCloudStatus('Sincronizado', true, `Actualizado ${formatTime(new Date())}`);
        return;
      } catch (error) {
        console.warn('La eliminación quedará pendiente:', error);
      }
    }
...
async function syncPendingParts(options = {}) {
  if (!window.LubaydOffline?.listPartQueue || !navigator.onLine) return { synced: 0, errors: 0 };
  const identity = await window.LubaydOffline.getDeviceIdentity?.();
  const deviceReady = Boolean(identity?.enrolled && identity.deviceId && identity.deviceToken);
  const directSessionReady = hasOnlineFirebaseSession() && window.LubaydCloud?.available;
  if (!deviceReady && !directSessionReady) return { synced: 0, errors: 0 };
  const items = await window.LubaydOffline.listPartQueue({ statuses: ['pending', 'error'] });
  let synced = 0;
  let errors = 0;
  for (const item of items) {
    try {
      ...
      let remote = null;
      const sameOnlineOperator = hasOnlineFirebaseSession()
        && authenticatedUser?.uid === item.userId
        && window.LubaydCloud?.available;

      if (sameOnlineOperator) {
        try {
          if (item.operation === 'delete') {
            await window.LubaydCloud.remove(item.recordId);
          } else {
            await window.LubaydCloud.save(item.record);
            remote = item.record;
          }
        } catch (directError) {
          console.warn('Sincronización directa del parte falló; se intenta mediante el dispositivo:', directError);
          remote = await window.LubaydOffline.syncPartQueueItemWithDevice(item, identity);
        }
      } else {
        if (!deviceReady) throw new Error('El dispositivo no tiene una credencial válida para sincronizar este parte.');
        remote = await window.LubaydOffline.syncPartQueueItemWithDevice(item, identity);
      }
      ...
      if (item.operation === 'upsert') state.updateLocal(item.recordId, { syncStatus: 'error', syncError: error.message || String(error) });
    }
  }
  renderAll();
  if ($('#historial')?.classList.contains('active')) renderHistory();
  if (!options.silent && (synced || errors)) {
    showToast(errors ? 'Sincronización incompleta' : 'Partes sincronizados', errors ? `${synced} enviados y ${errors} con error.` : `${synced} parte(s) enviados a Firebase.`);
  }
...
async function refreshOfflineAccess() {
  ...
  try {
    offlineProfiles = await window.LubaydOffline.listProfiles({ onlyWithPin: true });
    panel.classList.toggle('hidden', !offlineProfiles.length);
...
async function offlineLogin() {
  ...
  if (!uid || !pin) return setOfflineLoginMessage('Selecciona un usuario e ingresa el PIN.');
...
async function updateOfflinePreparationUi() {
  ...
  if (text) text.textContent = hasPin
    ...
    : 'Configura un PIN mientras tengas internet para poder abrir la asistencia sin conexión.';
  if (badge) {
    badge.textContent = hasPin ? 'Preparado' : 'Falta PIN';
...
$('#parteForm')?.addEventListener('submit', async event => {
  ...
  try {
    ...
    showToast('Parte guardado', hasOnlineFirebaseSession() ? 'El registro quedó sincronizado.' : 'El registro quedó guardado en este teléfono y se enviará al recuperar internet.');
    showView('dashboard');
  } catch (error) {
...
function renderHistory() {
  ...
  body.innerHTML = records.map(record => `
    <tr>
      <td>${formatDate(record.fecha)}${partSyncBadge(record)}</td>
  ...
  cards.innerHTML = records.map(record => `
    <article class="history-card">
      <div class="history-card-head">
        <div><strong>${escapeHtml(record.monte)} · ${escapeHtml(record.maquina)}</strong><span>${formatDate(record.fecha)} · ${escapeHtml(record.operador)}</span>${partSyncBadge(record)}</div>
...
window.addEventListener('online', () => {
  syncNetworkStatus();
  updateOfflinePreparationUi();
  syncPendingParts({ silent: true }).catch(console.warn);
  if (!offlineSession && window.LubaydCloud?.available && !cloudUnsubscribe) startCloudSync();
});
window.addEventListener('load', () => {
  if (navigator.onLine) window.setTimeout(() => syncPendingParts({ silent: true }).catch(console.warn), 2200);
});
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=20.8.2', { scope: './', updateViaCache: 'none' });

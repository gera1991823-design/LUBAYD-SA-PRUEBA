...
async function handleAuthChange(user) {
  ...
  try {
    ...
    window.setTimeout(() => { syncPendingParts({ silent: true }).catch(console.warn); window.LubaydSyncFuel?.({ silent: true })?.catch(console.warn); }, 5000);
  } catch (error) {
    interactiveAuthIntent = false;
    window.LubaydInteractiveLogin = false;
...
const viewMeta = {
  ...
  combustibleControl: ['Control de combustible', 'Parte único de combustible'],
...
function restorePreferredView() {
  ...
  const offlineAllowed = new Set(['dashboard', 'asistencia', 'nuevo', 'combustibleControl', 'historial', 'graficos', 'ubicaciones']);
...
function showView(id, options = {}) {
  if (!currentUser()) return;
  if (offlineSession) {
    const offlineViews = new Set(['dashboard', 'asistencia', 'nuevo', 'combustibleControl', 'historial', 'graficos', 'ubicaciones']);
    if (!offlineViews.has(id)) {
      showToast('Función en línea', 'Esta sección necesita conexión. Puedes usar Inicio, Partes, Combustible, Historial, Gráficos, Ubicaciones y Asistencia sin internet.');
      return;
    }
  }
  ...
  if (id === 'combustibleControl' && window.LubaydFuelUI?.show) window.LubaydFuelUI.show();
...
function renderHistory() {
  ...
  cards.innerHTML = records.map(record => `
    <article class="history-card">
      ...
      <div class="history-card-meta"><span>${escapeHtml(record.actividad || 'Sin actividad')}</span><span>${formatNumber(record.horas, 1)} h</span>${Number(record.combustible || 0) > 0 ? `<span>${formatNumber(record.combustible, 1)} L</span>` : ''}</div>
...
function openDetail(id) {
  ...
  const fields = {
    ...
    ...(Number(record.combustible || 0) > 0 ? { Combustible: `${formatNumber(record.combustible, 1)} L` } : {}),
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=21.1.0', { scope: './', updateViaCache: 'none' });

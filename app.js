...
async function syncPendingParts(options = {}) {
  ...
  if (!identity?.enrolled || !identity.deviceId || !identity.deviceToken) return { synced: 0, errors: 0 };
  ...
  for (const item of items) {
    try {
      ...
      const remote = await window.LubaydOffline.syncPartQueueItemWithDevice(item, identity);
...
async function handleAuthChange(user) {
  ...
  try {
    ...
    updateOfflinePreparationUi();
    window.setTimeout(() => window.LubaydSyncOfflineAttendance?.({ silent: true }), 1000);
  } catch (error) {
    interactiveAuthIntent = false;
    window.LubaydInteractiveLogin = false;
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=20.8.0', { scope: './', updateViaCache: 'none' });

...
async function syncPendingParts(options = {}) {
  ...
  syncPendingPartsPromise = (async () => {
    if (!window.LubaydOffline?.listPartQueue || !navigator.onLine || !authenticatedUser?.uid) return { synced: 0, errors: 0 };
      ...
      try {
        ...
        if (sameOnlineOperator) {
          ...
          try {
            ...
            console.warn('Function de partes no disponible; se intenta Firestore directo:', functionError);
            if (!window.LubaydCloud?.available) throw functionError;
        ...
        if (!navigator.onLine || /network|conexi|offline|fetch|abort/i.test(String(error?.message || error))) break;
      }
    }
...
function readNavigationState() {
  try {
    const parsed = JSON.parse(sessionStorage.getItem(NAVIGATION_STATE_KEY) || '{}');
...
function writeNavigationState(changes = {}) {
  ...
  try { sessionStorage.setItem(NAVIGATION_STATE_KEY, JSON.stringify(next)); } catch (_) {}
  return next;
}
...
window.addEventListener('online', () => {
  syncNetworkStatus();
  updateOfflinePreparationUi();
  authorizeCurrentDeviceForOperator({ silent: true }).catch(() => null).finally(() => scheduleAutomaticSync(250));
  scheduleAutomaticSync(2500);
  if (!offlineSession && window.LubaydCloud?.available && !cloudUnsubscribe) startCloudSync();
});
window.addEventListener('load', () => {
  if (navigator.onLine) scheduleAutomaticSync(2200);
});
window.addEventListener('pageshow', () => {
  if (navigator.onLine) scheduleAutomaticSync(700);
});
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible' && navigator.onLine) scheduleAutomaticSync(500);
});
window.addEventListener('offline', () => { syncNetworkStatus(); updateOfflinePreparationUi(); });
periodicSyncTimer = window.setInterval(() => {
  if (navigator.onLine && currentUser()) scheduleAutomaticSync(0);
}, 30000);
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=20.8.6', { scope: './', updateViaCache: 'none' });

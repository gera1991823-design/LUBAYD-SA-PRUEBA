/* Lubayd SA V22.0.0 - aplicación principal responsive */
(function () {
  'use strict';
  const { $, $$, state, config, initials, formatDateTime, escapeHtml, toast, emit, localDateKey, formatNumber } = window.Lubayd;
  const viewTitles = {
  ...
  function showView(name) {
    ...
    sessionStorage.setItem('lubayd_last_view_v22_0', name);
  ...
  async function updateOfflineStatus() {
    ...
    const rows = [
      ...
      ['IndexedDB disponible', status.indexedDb], ['Registros pendientes', status.pending || 0], ['Preparado el', status.preparedAt ? formatDateTime(status.preparedAt) : '—']
    ];
  ...
  async function renderDashboard() {
    ...
    const lastSync = localStorage.getItem('lubayd_last_sync_v22');
  ...
  async function synchronize() {
    ...
    try {
      ...
      if (result.errors.length) toast('Sincronización parcial', `${result.synced} enviados y ${result.errors.length} con error.`);
        ...
        localStorage.setItem('lubayd_last_sync_v22', new Date().toISOString());
  ...
  function enterSession(detail) {
    ...
    const requested = new URLSearchParams(location.search).get('view') || sessionStorage.getItem('lubayd_last_view_v22_0') || 'dashboard';
  ...
  async function registerServiceWorker() {
    ...
    try {
      const registration = await navigator.serviceWorker.register('./service-worker.js?v=22.0.0', { scope: './' });
  ...
  function bind() {
    ...
    $('#refreshOfflineButton').addEventListener('click', updateOfflineStatus); $('#forceUpdateButton').addEventListener('click', forceUpdate);

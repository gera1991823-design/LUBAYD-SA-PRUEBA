...
(function () {
  ...
  try {
    ...
    const fuelLoadsCollection = db.collection('combustible_cargas');
    const fuelMovementsCollection = db.collection('combustible_movimientos');
    const fuelFlowStateCollection = db.collection('combustible_flujo_estado');
    const fuelFlowMovementsCollection = db.collection('combustible_flujo_movimientos');
    const fuelFlowPhotosCollection = db.collection('combustible_flujo_fotos');
    ...
    window.LubaydFuelCloud = {
      available: true,
      subscribe(onData, onError) {
        if (!auth.currentUser) throw new Error('Debes iniciar sesión para consultar combustible.');
        let loads = [];
        let movements = [];
        let loadReady = false;
        let movementReady = false;
        const emit = () => {
          if (!loadReady || !movementReady) return;
          onData({ loads, movements });
        };
        const unsubscribeLoads = fuelLoadsCollection.onSnapshot(function (snapshot) {
          loads = snapshot.docs.map(function (doc) {
            return Object.assign({ id: doc.id }, normalizeFirestoreValue(doc.data()));
          }).sort(function (a, b) {
            return String(b.createdAt || b.dateTime || '').localeCompare(String(a.createdAt || a.dateTime || ''));
          });
          loadReady = true;
          emit();
        }, onError);
        const unsubscribeMovements = fuelMovementsCollection.onSnapshot(function (snapshot) {
          movements = snapshot.docs.map(function (doc) {
            return Object.assign({ id: doc.id }, normalizeFirestoreValue(doc.data()));
          }).sort(function (a, b) {
            return String(b.createdAt || b.dateTime || '').localeCompare(String(a.createdAt || a.dateTime || ''));
          });
    ...
    window.LubaydFuelFlowCloud = {
      ...
      subscribe(onData, onError) {
        ...
        let movementReady = false;
        const emit = () => {
          if (!stateReady || !movementReady) return;
          onData({ state: flowState, movements });
        };
        const unsubscribeState = fuelFlowStateCollection.doc('actual').onSnapshot(function (snapshot) {
          flowState = snapshot.exists ? Object.assign({ id: snapshot.id }, normalizeFirestoreValue(snapshot.data())) : null;
          stateReady = true;
          emit();
        }, onError);
        const unsubscribeMovements = fuelFlowMovementsCollection.orderBy('dateTime', 'desc').limit(300).onSnapshot(function (snapshot) {
          movements = snapshot.docs.map(function (doc) {
            return Object.assign({ id: doc.id }, normalizeFirestoreValue(doc.data()));
          });
          movementReady = true;
      ...
      async getPhoto(photoId) {
        if (!auth.currentUser) throw new Error('Debes iniciar sesión para consultar el comprobante.');
        if (!photoId) throw new Error('Fotografía no válida.');
        const snapshot = await fuelFlowPhotosCollection.doc(photoId).get();
        if (!snapshot.exists) throw new Error('La fotografía no existe en el servidor.');
    ...
    const OFFLINE_DEVICE_AUTHORIZE_URL = 'https://southamerica-east1-lubayd-sa.cloudfunctions.net/authorizeOfflineDevice';
    const offlineDevicesCollection = db.collection('offline_devices');

    window.LubaydOfflineDeviceCloud = {
      available: true,
      async authorizeCurrentUserDevice(payload) {
        if (!navigator.onLine) throw new Error('Necesitas internet para autorizar este teléfono.');
        const user = requireAuthenticated();
        const profile = window.LubaydCurrentProfile || {};
        if (profile.active !== true || profile.role !== 'operador') throw new Error('Solo un operador activo puede autorizar este teléfono.');
        const deviceId = String(payload?.deviceId || '').trim();
        const deviceToken = String(payload?.deviceToken || '').trim();
        const deviceName = String(payload?.deviceName || 'Dispositivo Lubayd').trim().slice(0, 100);
        if (!/^device_[A-Za-z0-9_-]{10,}$/.test(deviceId)) throw new Error('El identificador del dispositivo no es válido.');
        if (deviceToken.length < 30 || deviceToken.length > 200) throw new Error('La credencial del dispositivo no es válida.');
        const idToken = await user.getIdToken(true);
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 30000);
        try {
          const response = await fetch(OFFLINE_DEVICE_AUTHORIZE_URL, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${idToken}`
            },
            body: JSON.stringify({ deviceId, deviceToken, deviceName }),
            signal: controller.signal
          });
          let result = {};
          try { result = await response.json(); } catch (_) { result = {}; }
          if (!response.ok || result.ok !== true) throw new Error(result.error || `No se pudo autorizar el teléfono (${response.status}).`);
          return result;
        } catch (error) {
          if (error?.name === 'AbortError') throw new Error('La autorización demoró demasiado. Vuelve a intentar.');
          throw error;
        } finally {
          clearTimeout(timer);
        }
      ...
      async enrollDevice(payload) {
        ...
        const deviceName = String(payload?.deviceName || 'Dispositivo Lubayd').trim().slice(0, 100);
        const allowedUserIds = Array.from(new Set((payload?.allowedUserIds || []).map(String).filter(Boolean))).slice(0, 500);
        if (!/^device_[A-Za-z0-9_-]{10,}$/.test(deviceId)) throw new Error('El identificador del dispositivo no es válido.');
        if (!/^[a-f0-9]{64}$/.test(tokenHash)) throw new Error('La credencial del dispositivo no es válida.');
        if (!allowedUserIds.length) throw new Error('Prepara al menos un operador para acceso sin conexión.');
        await offlineDevicesCollection.doc(deviceId).set({
          deviceId,
          deviceName,
          tokenHash,
          active: true,
          allowedUserIds,
          preparedByUid: admin.uid,
          preparedByEmail: admin.email || '',
          updatedAt: serverTimestamp(),
          updatedAtClient: new Date().toISOString(),
          createdAtClient: new Date().toISOString()
        }, { merge: true });
        return { deviceId, deviceName, allowedUserIds };
      },
      async revokeDevice(deviceId) {
        requireAdmin();
        if (!deviceId) throw new Error('Dispositivo no válido.');
        await offlineDevicesCollection.doc(deviceId).set({
          active: false,
      ...
      async syncQueueItem(item, identity) {
        if (!navigator.onLine) throw new Error('El dispositivo todavía no tiene conexión a internet.');
    ...
    window.LubaydFuelFlowCloud = { available: false, error: error };
